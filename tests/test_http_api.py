...
class TestHttpApi(testing.TestCase):
    ...
    def _get_user_via_http(self, login: str, version, requester_login: str = None):
        """Helper
        """
        url = http_api.url('auth@get_user', {'uid': auth.get_user(login).uid}, version)
    ...
    def test_patch_user(self):
        ...
        url = http_api.url('auth@patch_user', {'uid': user.uid})
    ...
    def test_post_follow_delete_follow(self):
        ...
        for version in (1, 2):
            ...
            # Follow
            url = http_api.url('auth@post_follow', {'uid': user2.uid}, version)
            ...
            # Unfollow
            url = http_api.url('auth@delete_follow', {'uid': user2.uid}, version)
    ...
    def test_get_following_count_get_follows_get_followers(self):
        ...
        for version in (2,):
            ...
            url = http_api.url('auth@get_follows', {'uid': user1.uid}, version)
            ...
            url = http_api.url('auth@get_followers', {'uid': user2.uid}, version)
            ...
            url = http_api.url('auth@get_followers', {'uid': user3.uid}, version)
    ...
    def test_get_blocked_users(self):
        ...
        for version in (2,):
            ...
            url = http_api.url('auth@get_blocked_users', {'uid': user1.uid}, version)
            resp = self.send_http_request(self.prepare_http_request('GET', url, {'PytSite-Auth': user1_token}))
            self.assertHttpRespCodeEquals(resp, 200)
            self.assertHttpRespJsonFieldIsInt(resp, 'remains')
            self.assertHttpRespJsonFieldListLen(resp, 'result', 2)
    ...
    def test_post_block_user_delete_block_user(self):
        ...
        for version in (1, 2):
            ...
            url = http_api.url('auth@post_block_user', {'uid': user2.uid}, version)
            resp = self.send_http_request(self.prepare_http_request('POST', url, {'PytSite-Auth': user1_token}))
            if version == 1:
            ...
            # Unblock
            url = http_api.url('auth@delete_block_user', {'uid': user2.uid}, version)
            resp = self.send_http_request(self.prepare_http_request('DELETE', url, {'PytSite-Auth': user1_token}))
            if version == 1:
